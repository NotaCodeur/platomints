...
import { Layout, Typography, Col, Row, Card, Button, Input, List, Divider, Space, Dropdown, Anchor } from 'antd';
...
const App = () => {
    ...
    const reducerTwo = (stateTwo, action) => {
        switch (action.type) {
            ...
            case 'setHasAdminNFT':
                // console.log('dispatch setUserHasNFT', action.payload);
                return { ...stateTwo, hasAdminNFT: action.payload }

            case 'setUserNFTArray':
                // console.log('dispatch setUserHasNFT', action.payload);
                return { ...stateTwo, userNFTArray: action.payload }


            case 'setUserCollectiblesArray':
                console.log('stateTwo.userCollectiblesArray', stateTwo.userCollectiblesArray);
                console.log('action.payload', action.payload);
                // console.log('stateTwo.userCollectiblesArray.includes(action.payload)', stateTwo.userCollectiblesArray.inlcudes(action.payload));

                return { ...stateTwo, userCollectiblesArray: action.payload }

                // if (stateTwo.userCollectiblesArray.includes(action.payload)) {
                //     return { ...stateTwo, userCollectiblesArray: [...stateTwo.userCollectiblesArray] }
                // } else {
                //     return { ...stateTwo, userCollectiblesArray: [...stateTwo.userCollectiblesArray, action.payload] }

                // }
            // case 'setUserCollectiblesArray':
            // console.log('dispatch userCollectiblesArray', action.payload);
            // let query = action.payload?.policyAndName
            // console.log('query', query) 
            // try {
            //     const response = await axios.get(`/.netlify/functions/${action.payload}`)

            //     return { ...stateTwo, userCollectiblesArray: [...stateTwo.userCollectiblesArray, { ...action.payload, asset: JSON.stringify(response.data) }] }
  ...
  */
    ...
    const getWalletIcon = async () => {
        try {
            const walletKey = stateTwo.whichWalletSelected;
            const walletIcon = window?.cardano?.[walletKey].icon;
            dispatchTwo({ type: 'setwalletIcon', payload: walletIcon })
    ...
    useEffect(() => {
        console.log(stateTwo.userNFTArray)
    ...
    useEffect(() => {
        ...
        let userHasAdminNFT = false;
        let userNFTArray = []
        if (stateTwo.Utxos?.length) {
            console.log('Utxos: ', stateTwo.Utxos)
            stateTwo.Utxos?.map((Utxo) => {
                // console.log('Utxo in map: ', Utxo)
                // f5ae502dd088b3e2a0e3550f6eea9d6a8a9d847e29c631f4467fbb0a
                // if (Utxo?.multiAssetStr?.includes('5dac8536653edc12f6f5e1045d8164b9f59998d3bdc300fc92843489')) {
                if (Utxo?.multiAssetStr?.includes('f5ae502dd088b3e2a0e3550f6eea9d6a8a9d847e29c631f4467fbb0a')) {
                    // console.log('Utxo includes NMKR Yay!, set hasNFT to true')
                    userHasNFT = true
                }

                // liminal NFT
                if (Utxo?.multiAssetStr?.includes('9f8bd415555c49d1966a2156cd5feaabbda675e977b52f03b131fcd2')) {
                    // console.log('Utxo includes NMKR Yay!, set hasNFT to true')
                    userHasNFT = true
                    userHasAdminNFT = true
                }

                if (Utxo.multiAssetStr) {
                    console.log('multiAssetStr', Utxo.multiAssetStr)
                    var assetArray = Utxo.multiAssetStr.split(')')

                    assetArray.forEach((assetString) => {

                        if (assetString.length >= 1) {
                            var assetStringArray = assetString.split('+')

                            var NFTName = assetStringArray[2]?.split('(')[1]
                            var NFTAmount = assetStringArray[1]?.replace(/\s/g, "")
                            var NFTPolicyID = assetStringArray[2]?.split('(')[0]?.replace(/\s/g, "")?.split('.')[0]
                            var NFTPolicyIDAndName = assetStringArray[2]?.split('(')[0]?.replace(/\s/g, "")?.split('.')[0] + assetStringArray[2]?.split('(')[0]?.replace(/\s/g, "")?.split('.')[1]

                            var NFTObject = {
                                name: NFTName,
                                amount: NFTAmount,
                                policy: NFTPolicyID,
                                policyAndName: NFTPolicyIDAndName
                            }
                            userNFTArray.push(NFTObject)
                            console.log('assetStringArray', assetStringArray)
                            console.log('NFTAmount', NFTAmount)
                            console.log('NFTName', NFTName)
                            console.log('NFTPolicyID', NFTPolicyID)
                            console.log('NFTPolicyIDAndName', NFTPolicyIDAndName)
                        }
                    })
                }
            })
        ...
        dispatchTwo({ type: 'setHasAdminNFT', payload: userHasAdminNFT })
        dispatchTwo({ type: 'setUserNFTArray', payload: userNFTArray })
