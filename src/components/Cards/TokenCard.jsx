...
const TokenCard = ({ NFT, artSize, stateTwo, dispatchTwo }) => {
    // console.log(gameObj)
    ...
    useEffect(() => {
        if (skip1 && !stateTwo?.userCollectiblesArray?.map((nftObj) => nftObj.policy).includes(NFT.policy)) {
            setSkip1(false)
        }
    ...
    useEffect(() => {
        ...
        //     setSkip1(true)
        // }
        if (CNFTAsset === undefined && !stateTwo?.userCollectiblesArray?.map((nftObj) => nftObj.policy).includes(NFT.policy)) {
            setPolicyAndName(NFT.policyAndName)
            ...
            console.log('stateTwo.userCollectiblesArray in tokenCard', stateTwo.userCollectiblesArray)
            console.log('NFT in tokenCard', NFT)
            console.log('stateTwo.userCollectiblesArray.includes(NFT.policy) in tokenCard', stateTwo?.userCollectiblesArray?.map((nftObj) => nftObj.policy).includes(NFT.policy))
            if (stateTwo?.userCollectiblesArray?.length && stateTwo?.userCollectiblesArray?.map((nftObj) => nftObj.policy
            ).includes(NFT.policy)) {
                console.log('stateTwo.userCollectiblesArray.includes({ ...NFT, asset: CNFTAsset })', stateTwo?.userCollectiblesArray?.map((nftObj) => nftObj.policy
                ).includes(NFT.policy))

            }
            else {

                dispatchTwo({ type: 'setUserCollectiblesArray', payload: [...stateTwo.userCollectiblesArray, { ...NFT, asset: CNFTAsset }] })
            }
        }
    ...
    return (
        <>
            <Card
                ...
                // cover={<img alt="example" src={'https://api.ipfsbrowser.com/ipfs/get.php?hash=' + CNFTAsset?.last_metadata?.image?.split('//')[1]} className='artSectionImg' style={{
                //     height: artSize,
                //     // backgroundSize: 'cover',
                //     // objectFit: 'cover',
